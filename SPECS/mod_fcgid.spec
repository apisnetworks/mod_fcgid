...
install -d -m 755 %{buildroot}%{_tmppath}/fcgi

# Include the manual as %%doc, don't need it elsewhere
rm -rf %{buildroot}/usr/share/httpd/manual

# Make sure %%{_tmppath}/fcgi exists at boot time for systems
# with %%{_tmppath} on tmpfs (#656625)
%if 0%{?fedora} > 14 || 0%{?rhel} > 6
...
if [ $1 -eq 0 ]; then
	...
	[ -d %{_tmppath}/fcgi ] && \
		/sbin/restorecon -R %{_tmppath}/fcgi &> /dev/null || :
fi
exit 0
%endif
...
%endif
%dir %attr(0755,apache,apache) %{_tmppath}/fcgi/
